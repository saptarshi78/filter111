...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        btn = []
        search = message.text
        mo_tech_yt = ** Here Is Your Query:** #{search}**"
        ...
        if files:
            ...
            await client.send_message(chat_id=message.from_user.id, message=Bro, Check Movie Name In #Google and Try ! Then No Results Add Movie Year and Try , Again No Results ? It's Not Available In Our Database Or Movie Not Released !
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        btn = []
        search = message.text
        mo_tech_yt = ** 📁 Here is What I Found In My Database For Your Query : #{search} 👇 **📤 Uploaded by @File_Search_V3_BOT 🔍 **"
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            buttons = [
                [
                    InlineKeyboardButton('Update Channel ⚙️', url=f'{TUTORIAL}')
                ]
                ]
            ...
            for files in filedetails:
                ...
                buttons = [
                    [
                        InlineKeyboardButton('🖥️ Update Channel 🖥️', url=f'{TUTORIAL}')
                    ]
                    ]
